<?php
	...
	define('RECEIVER','admin@chodatso.com'); // Email tài khoản Ngân Lượng
	define('MERCHANT_ID', '48091'); // Mã kết nối
	define('MERCHANT_PASS', '24ef2a0e943f94b70070f18c01efaf7f'); // Mật khẩu kết nối
        //end require_once('config.php');
	...
	//require_once('include/lib/nusoap.php');
        ...
        */
        ...
        //end require_once('include/lib/nusoap.php');
	...
	class NL_MicroCheckout
	{
        ...
        //end require_once('include/nganluong.microcheckout.class.php');
	//khai bao
...
if (isset($result) && !empty($result)) {
    if ($result['result_code'] == '00') {
        $checkExist = DB::table('giaodichs')->where('code', '=', $_GET['token_code'])->get();
        //dd(count($checkExist) < 1);
        //dd(count($checkExist));
        ...
        if(count($checkExist) < 1){
            $user->coins = $user->coins + $result['amount'] / 1000;
            $user->save();
            DB::table('giaodichs')->insert(
            [
                    'user' => $user_id,
                    'description' => 'Nạp ' . $result['amount'] . ' vào tài khoản',
                    'new' => $user->coins,
                    'code' => $_GET['token_code'],
...
<div class="text-center text-bold">Quý khách đã nạp tiền thành công! Giờ Quý khách có <?php echo $user->coins; ?> Coins trong tài khoản </div>
<?php			
    } else {
